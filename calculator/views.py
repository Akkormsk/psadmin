...
def _persist_estimate(request, estimate, settings):
    """Parse the posted calculator state and write it to ``estimate``
    (creating one when it is ``None``). Returns ``(estimate, error)`` —
    ``error`` is a message string when nothing was saved. Shared by the
    plain form POST and the autosave endpoint."""
    try:
        ...
        return estimate, "Проверьте тираж, часы и добавленные строки."
    ...
    for row in raw_lines:
        try:
            ...
            return estimate, "Одна из строк расчёта заполнена неверно."
...
def home(request, pk=None):
    ...
    if request.method == "POST":
        posted_type = request.POST.get("calculator_type")
        if posted_type in {Estimate.TYPE_SHEET, Estimate.TYPE_WIDE}:
            calculator_type = posted_type
        estimate, error = _persist_estimate(request, estimate, settings)
        if error:
...
def save_estimate(request, pk=None):
    """Autosave: same write as the form POST, but returns JSON and never
    redirects — the calculator page keeps its state."""
    estimate = None
    if pk:
        estimate = _estimate_for_user(request, pk)
        if estimate is None:
            raise Http404
    estimate, error = _persist_estimate(request, estimate, _settings())
    if error:
        return JsonResponse({"error": error}, status=400)
    return JsonResponse({
        "pk": estimate.pk,
        "url": reverse("calculator_estimate", args=[estimate.pk]),
        "saved_at": timezone.localtime(estimate.updated_at).strftime("%H:%M"),
        "standard": estimate.summary_snapshot.get("standard", ""),
    })
...
def duplicate_estimate(request, pk):
    ...
    copy = Estimate.objects.create(
        ...
        name=f"{original.name} (копия)"[:200],
    ...
    EstimateLine.objects.bulk_create([
        EstimateLine(
            estimate=copy, category=line.category, price_item=line.price_item,
            name_snapshot=line.name_snapshot, unit_name=line.unit_name,
            unit_price_snapshot=line.unit_price_snapshot, quantity=line.quantity,
            is_custom=line.is_custom,
        )
        for line in lines
    ])
    messages.success(request, f"Создана копия расчёта «{original.name}».")
    return redirect("calculator_estimate", pk=copy.pk)
