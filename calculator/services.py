...
def calculate_sheet_estimate(lines, product_quantity, work_hours, settings):
    material_cost = sum((line["quantity"] * line["unit_price"] for line in lines), ZERO)
    paper_sheets = sum((line["quantity"] for line in lines if line["category"] == PriceItem.CATEGORY_PAPER), ZERO)
