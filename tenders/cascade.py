...
Правила:
- criteria: РОВНО одна запись на каждую пронумерованную строку ТЗ, в ТОМ ЖЕ порядке, без пропусков. Даже если две строки означают одно и то же по смыслу (например «синий» и «цвет: синий») — НЕ объединяй их в одну запись, верни для каждой свою запись с одинаковым разобранным значением. Строгое соответствие "одна строка → одна запись" важнее компактности: от него зависит, что шаг 5 сравнит требование именно с тем полем карточки, о котором оно было. Характеристикой может быть ЛЮБОЕ измеримое или описываемое свойство товара — не ограничивайся заранее известным списком (плотность, яркость, мощность, температура, вязкость — что угодно).
