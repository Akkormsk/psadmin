...
class Cascade:
    ...
    def step_1_parse_tz(self) -> list[Criterion]:
        ...
        criteria_cache_key = f"criteria-v4|{self._tz_hash}|{model}"
...
- importance: 0..100 — НЕ общая важность требования для бизнеса, а приоритет ИМЕННО для проверки конкретного товара по каталогу: насколько велика роль этого критерия при сравнении карточки товара с этой строкой ТЗ. Требования, которые в принципе не с чем сравнить в карточке каталога (маркировка Честного Знака, документы, сроки поставки, гарантия, условия производства и пошива) — низкий приоритет (не выше 20), даже если по факту они важны для сделки: сверять их с товаром всё равно нечем. importance_reason — короткое объяснение именно с точки зрения проверки по каталогу, а не общей значимости.
