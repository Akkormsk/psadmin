...
Если смысл parent совпадает с искомой товарной сущностью, а children лишь делят её на варианты или подгруппы, parent может быть primary независимо от меньшей глубины и более короткого названия; не классифицируй такой parent как fallback только из-за краткости. Узел с тем же смыслом без дополнительного ограничения может быть equivalent. Для каждого source с primary верни fallback, если в переданной ветке есть обоснованный более широкий узел за пределами самой товарной сущности.
Обязательная самопроверка перед ответом: для каждого предполагаемого primary сравни его с parent и всеми непосредственными children. Если parent сам обозначает искомую товарную сущность, а все его children являются только вариантами этой же сущности, назначь primary узлу покрытия, а не одному варианту. Такой parent не является fallback; fallback должен быть шире самой товарной сущности.
