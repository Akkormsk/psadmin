"""Резервный канал скачивания документов извещения — официальный SOAP-сервис ЕИС
(getDocsIP, «получатель машиночитаемых данных»), метод getDocsByReestrNumberRequest.

Используется, когда публичная ссылка на файл (zakupki.gov.ru/.../filestore/...) не
отвечает — это стало происходить регулярно, портал явно режет автоматические скачивания.
Публичную ссылку по-прежнему пробуем первой (быстро, без токена и лимитов); сюда
проваливаемся только при её отказе — сервис недокументирован по лимитам, экономим запросы.
