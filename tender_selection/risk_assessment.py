...
Запускается лениво при открытии карточки тендера (см. services.risk_assessment_for),
результат кэшируется на FoundTender — не пересчитывается на каждый заход.
...
REQUIRED_KEYS = (
    ...
    "penalties", "national_regime", "sample_requirements", "legal_risks",
)
